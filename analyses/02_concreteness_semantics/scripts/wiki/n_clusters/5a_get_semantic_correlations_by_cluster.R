# get pairwise correlations between words across languages for each language pair for wikidata 250 solution by cluster
...
OUTPATH <-  here("analyses/02_concreteness_semantics/data/wiki/mean_cluster_corrs_250/pairwise_semantics_correlations_wiki_by_nclusters_250_")
...
write_corrs <- function(current_n_clusters,
  ...
  mean_corrs <- corr_values %>%
    ...
    group_by(n_clusters, same, cluster1) %>% # get cluster pair correlations
  ...
  full_outfile <- paste0(this_outfile, this_lang1, "_", this_lang2, "_by_cluster.csv")
