...
write_distance_chunk <- function(this_cluster1,
  ...
  output_path <- paste0(pairwise_dist_prefix, "semantics_split_ward/",
                        current_lang, "_cluster_", this_cluster1,
                        "_", this_cluster2, "_pairwise_distances_ets_ward.feather")
...
break_up_pairwise_distances <- function(current_lang,
  ...
  pairwise_dists <-  read_feather(paste0(here("analyses/02_concreteness_semantics/data/ets/pairwise_distances/"),
...
clusters_to_loop <- crossing(cluster1 = 1:N_SEMANTIC_CLUSTERS,
                             cluster2 = 1:N_SEMANTIC_CLUSTERS) %>%
  arrange(cluster1)  %>%
